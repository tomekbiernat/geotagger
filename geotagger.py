...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument('--zoom', type=int, choices=range(0, 20), metavar='[0-19]', help=f'Map zoom in range 0-19 (default: {DEFAULT_ZOOM})', default=DEFAULT_ZOOM)
    parser.add_argument('--area', type=float, metavar='[0.1-5]', help=f'Area of the map 0.1-5 (default: {DEFAULT_AREA})', default=DEFAULT_AREA)
    args = parser.parse_args()
    if args.area < 0.1 or args.area > 5:
        print('Area has to be in range [0.1 - 5]')
        exit(1)
    args.file_names = []
...
def get_geodata(image: Image) -> GeoData | None:
    ...
    if exif_data and GPS_INFO_TAG in exif_data:
        ...
        lat = (-1 if gps_data[GPS_LAT_REF_TAG]  == 'S' else 1) * (lat_coords[0] + (lat_coords[1] + lat_coords[2] / 60.0) / 60.0)
        lon_coords = gps_data[GPS_LON_TAG]
        lon = (-1 if gps_data[GPS_LON_REF_TAG]  == 'W' else 1) * (lon_coords[0] + (lon_coords[1] + lon_coords[2] / 60.0) / 60.0)
        ret = GeoData(lat, lon)
...
def download_tile(tile: TileRef) -> Image:
    img_data = requests.get(f'http://a.tile.openstreetmap.org/{tile.zoom}/{tile.x}/{tile.y}.png', headers=HEADERS)
...
def generate_map(geodata: GeoData, zoom: float, tile_area: float) -> Image:
    ...
    map = Image.new('RGB', ((tile_box.x_max - tile_box.x_min + 1) * TILE_WIDTH, (tile_box.y_max - tile_box.y_min + 1) * TILE_HEIGHT))
    for x in range(tile_box.x_min, tile_box.x_max + 1):
        for y in range(tile_box.y_min, tile_box.y_max + 1):
            ...
            map.paste(tile_img, ((x - tile_box.x_min) * TILE_WIDTH, (y - tile_box.y_min) * TILE_HEIGHT))
    crop_x = (tile_coord.x - tile_area / 2 - tile_box.x_min) * TILE_WIDTH
    crop_y = (tile_coord.y - tile_area / 2 - tile_box.y_min) * TILE_HEIGHT
    map = map.crop((crop_x, crop_y, crop_x + tile_area * TILE_WIDTH, crop_y + tile_area * TILE_HEIGHT))
    draw = ImageDraw.Draw(map)
    draw.ellipse((int((map.width - DOT_SIZE) / 2), int((map.height - DOT_SIZE) / 2), int((map.width + DOT_SIZE) / 2), int((map.height + DOT_SIZE) / 2)), outline='red', width=2)
    if geodata.dir:
        def draw_line(angle: float):
            angle = angle / 180.0 * math.pi
            draw.line([int(map.width / 2), int(map.height / 2), int(map.width * (1 + math.sin(angle) * LINE_IMG_FRAC) / 2), int(map.height * (1 - math.cos(angle) * LINE_IMG_FRAC) / 2)], fill='red', width=2)
        draw_line(geodata.dir - FOV_ALPHA / 2)
        draw_line(geodata.dir + FOV_ALPHA / 2)
        draw.arc([int(map.width * (1 - FOV_ARC_FRAC) / 2), int(map.height * (1 - FOV_ARC_FRAC) / 2), int(map.width * (1 + FOV_ARC_FRAC) / 2), int(map.height * (1 + FOV_ARC_FRAC) / 2)],
                 geodata.dir - FOV_ALPHA / 2 - 90, geodata.dir + FOV_ALPHA / 2 - 90, fill='red', width=2)
    return map

def add_map_to_image(path: Path, zoom: float=DEFAULT_ZOOM, tile_area: float=DEFAULT_AREA) -> None:
    try:
    ...
    if not geodata:
        ...
        return
    print(f'Generating map for {path}{"" if geodata.dir else " (direction not included)"}...')
...
for f in args.file_names:
    add_map_to_image(Path(f), args.zoom, args.area)
